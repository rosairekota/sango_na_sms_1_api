...
@ApiTags('Woman-inscriptions:')
@Controller('Woman-inscription')
export class WomanInscriptionController {
    ...
    @Post()
     async create(inscription : AddWomanInscriptionDto) : Promise<WomanInscriptionEntity>{
        return await this.womanInscriptionService.add(inscription);
     }
    
     @Delete('/:id')
     async remove(@Param('id',ParseIntPipe) id : number): Promise<WomanInscriptionEntity>{
         return await this.womanInscriptionService.delete(id)
     }
     @Patch('/:id')
     async edit(@Param('id',ParseIntPipe) id :number,inscription : UpdateWomanInscriptionDto) : Promise<WomanInscriptionEntity>{
